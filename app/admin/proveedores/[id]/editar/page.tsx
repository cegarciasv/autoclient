...
export default async function EditarProveedorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6 max-w-5xl mx-auto">
    <div className="space-y-3">
      <Link href={`/admin/proveedores/${id}`} className="inline-flex items-center gap-2 text-sm font-medium text-[#52657a] hover:text-[var(--brand-primary)]"><ArrowLeft className="size-4" /> Volver al expediente</Link>
      <p className="eyebrow">Directorio / Proveedores</p>
      <h1 className="text-[32px] font-bold tracking-tight text-[#142033]">Editar proveedor</h1>
      <p className="text-sm text-[#52657a]">Actualiza los datos de registro de {tercero.razonSocial}.</p>
    </div>
    <FormNuevoTercero tipo="PROVEEDOR" id={id} initial={tercero} />
  </div>;
}
