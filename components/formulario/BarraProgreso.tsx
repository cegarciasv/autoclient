    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between text-xs">
        <span className="text-slate-500 font-medium">
          ...
          {" "}—{" "}
          <span className="text-[#2563EB] font-semibold">{labelActual}</span>
        </span>
        <span className="font-bold text-[#2563EB]">{progreso}%</span>
      </div>

      {/* Barra de progreso elegante */}
      <div className="w-full h-1.5 rounded-full bg-slate-100">
        <div
          className="h-1.5 rounded-full bg-gradient-to-r from-[#2563EB] to-[#1E3A8A] transition-all duration-500"
              ...
              <div className="flex flex-col items-center gap-1.5 flex-1">
                <div
                  className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all
                    ${completado ? "bg-[#2563EB] border-[#2563EB] text-white shadow-sm" : ""}
                    ${actual     ? "bg-white border-[#2563EB] text-[#2563EB] shadow-md ring-4 ring-[#2563EB]/10" : ""}
                ...
                <span
                  className={`text-[10px] text-center leading-tight hidden sm:block
                    ${actual ? "text-[#2563EB] font-semibold" : "text-slate-400"}`}
                >
                  {label}
                </span>
              </div>
                ...
                <div className="flex-1 flex items-center" style={{ marginTop: "18px" }}>
                  <div
                    className={`h-0.5 w-full transition-all duration-500
                      ${paso < pasoActual ? "bg-[#2563EB]" : "bg-slate-200"}`}
                  />
                </div>
              )}
            </div>
